...
public class SuperSpin extends BaseClientRequestHandler {
    @Override
    public void handleClientRequest(User player, ISFSObject params) {

        Entrance gameExt = (Entrance) getParentExtension();
        gameExt.log.info( "[" + gameExt.zoneName + "]" + "[" + gameExt.roomName + "]" + "SpuerSpin," + player.getName() + "," + params.toJson());

        int state = 0;
        long totalWin = 0;

        //非免費時間押注
        if(!gameExt.gameState.equals(GameState.STATE_SUPER_GAME)){
            state = 1;
            gameExt.log.info( "[" + gameExt.zoneName + "]" + "[" + gameExt.roomName + "]" + "NO STATE_SUPER_SPIN");
            return;
        }

        // 押注成功
        totalWin = gameExt.getSuperResult(gameExt.reSpinBet);
        ...
        if(gameExt.superSpinCount == 0)
            superLevelWin = gameExt.getLevelWin(gameExt.logic.getTotalBet(gameExt.reSpinBet), gameExt.freeSpinWin);

        //押注
        ISFSObject resp = gameExt.setGameBet(player, GameState.STATE_SUPER_GAME.getId(), gameExt.userPoint, 0, 2);
        ...
        if(resp.getInt("Ret") == 0){
            ...
            //賽果
            try{
                resp = gameExt.setGameResult(player, GameState.STATE_SUPER_GAME.getId(), 0,
                ...
                if(resp.getInt("Ret") == 0){
                    ...
                    if(gameExt.gameState.equals(GameState.STATE_SUPER_GAME)){
                        ...
                        comResp = gameExt.setGameComplete(player, gameExt.logic.getTotalBet(gameExt.reSpinBet),
                                gameExt.seqOpenWin + gameExt.freeSpinWin, GameState.STATE_SUPER_GAME.getId());
        ...
        respon1.putInt("Count", gameExt.superSpinCount - gameExt.superToSuper);
        respon1.putUtfString("SuperSpinWin", gameExt.reSetPointToClient(gameExt.freeSpinWin));
        respon1.putIntArray("FreeLight", IntStream.of(gameExt.freeLight).boxed().collect(Collectors.toList()));
        respon1.putInt("FreeToFree", gameExt.superToSuper);
        respon1.putInt("SuperSpinCount", gameExt.superSpinCount);
        respon1.putInt("SuperTimes", gameExt.superTimes);
        respon1.putInt("BaseLevelWin", baseLevelWin);
        respon1.putInt("SuperLevelWin", superLevelWin);
        ...
        //送回客端
        gameExt.send("SuperSpinResult", respon1, player);
